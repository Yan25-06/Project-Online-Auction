...
export const ProductService = {
    ...
    appendDescription: async (id: string, additionalDescription: string) => await productModel.appendDescription(id, additionalDescription),

    findByCategory: async (categoryId: string, page: number = 1, limit: number = 20) => await productModel.findByCategory(categoryId, page, limit),

    search: async (query: string, categoryId?: string, sortBy?: string, page: number = 1, limit: number = 20) =>
        await productModel.search(query, categoryId, sortBy, page, limit),

    getEndingSoon: async (limit: number = 5) => await productModel.getEndingSoon(limit),

    getMostBids: async (limit: number = 5) => await productModel.getMostBids(limit),

    getHighestPrice: async (limit: number = 5) => await productModel.getHighestPrice(limit),

    findBySeller: async (sellerId: string, page: number = 1, limit: number = 20) =>
        await productModel.findBySeller(sellerId, page, limit),

    updateStatus: async (id: string, status: 'active' | 'ended' | 'sold' | 'cancelled') =>
    ...
    incrementViewCount: async (id: string) => await productModel.incrementViewCount(id),

    delete: async (id: string) => await productModel.delete(id),

    getNewlyListed: async (minutes: number = 60) => await productModel.getNewlyListed(minutes)
};