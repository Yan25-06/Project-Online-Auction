...
export const ratingModel = {
  ...
  async upsert(orderId: string, ratingUserId: string, ratedUserId: string, score: 'positive' | 'negative', feedback?: string): Promise<Rating> {
  ...
  async updateUserRatings(userId: string): Promise<void> {
    ...
    const positiveCount = data?.filter(r => r.score === 'positive').length || 0;
    const totalCount = data?.length || 0;
    const ratingScore = totalCount > 0 ? (positiveCount / totalCount) * 100 : null;
  ...
  async findByOrderAndRater(orderId: string, ratingUserId: string): Promise<Rating | null> {
    const { data, error } = await supabase
      .from('ratings')
      .select('*')
      .eq('order_id', orderId)
